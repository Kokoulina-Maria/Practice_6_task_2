            ...
            do
            {
                ...
                else
                {
                    ...
                    for (i=1; i<=N; i++)//находим и перебираем N элементов
                    {
                        ...
                        if ((ak == 0) && (i < N)&& (i>2))//если данный элемент равен нулю и его номер не равен N, то следующий элемент не существует, так как на 0 делить нельзя
                        {
                    ...
                    Console.WriteLine();
                        if (i == N + 1) Console.WriteLine("Причина останова: в последовательности " + N + " элементов");
                        else if (ak<=M)
                            Console.WriteLine("Причина останова: "+ak + "<=" + M);
                }                
                Console.ReadLine();
            } while (true);
        }
